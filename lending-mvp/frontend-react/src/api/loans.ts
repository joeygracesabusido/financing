...
export const getLoans = async (customerId?: string) => {
  const query = customerId
    ? `query GetLoans($customerId: String!) { loans(customerId: $customerId) { id principal status customerId productId borrowerName productName termMonths approvedPrincipal approvedRate createdAt updatedAt disbursedAt outstandingBalance } }`
    : `query GetLoans { loans { id principal status customerId productId borrowerName productName termMonths approvedPrincipal approvedRate createdAt updatedAt disbursedAt outstandingBalance } }`
...
export const getLoan = async (loanId: string) => {
  const response = await fetch(`${API_URL}/graphql`, {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      query: `query GetLoan($id: ID!) { loan(id: $id) { id customerId productId principal termMonths approvedPrincipal approvedRate status createdAt updatedAt disbursedAt outstandingBalance } }`,
      variables: { id: loanId }
    }),
  })
  if (!response.ok) {
