...
export default function UsersPage() {
    ...
    const [form, setForm] = useState({ username: '', email: '', role: 'user', fullName: '' })
    ...
    const openCreate = () => { setEditId(null); setForm({ username: '', email: '', role: 'user', fullName: '' }); setError(''); setShowModal(true) }
    const openEdit = (u: User) => {
        setEditId(u.id)
        setForm({ username: u.username, email: u.email, role: u.role, fullName: u.fullName ?? '' })
        setError('')
        setShowModal(true)
    }
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            const input = { username: form.username, email: form.email, role: form.role, fullName: form.fullName }
            if (editId) {
