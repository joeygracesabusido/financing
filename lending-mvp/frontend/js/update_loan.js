document.addEventListener('DOMContentLoaded', () => {
    const API_URL = '/graphql'; // Use relative path for API
    const updateLoanForm = document.getElementById('update-loan-form');
    const formMessage = document.getElementById('form-message');

    // Get loan ID from URL
    const urlParams = new URLSearchParams(window.location.search);
    const loanId = urlParams.get('id');

    if (!loanId) {
        formMessage.textContent = 'Error: Loan ID not found in URL.';
        formMessage.className = 'mt-4 text-sm font-bold text-red-500';
        return;
    }

    // Input fields
    const borrowerIdInput = document.getElementById('borrower-id');
    const amountRequestedInput = document.getElementById('amount-requested');
    const termMonthsInput = document.getElementById('term-months');
    const interestRateInput = document.getElementById('interest-rate');
    const statusSelect = document.getElementById('status');

    // GraphQL Query to fetch a single loan
    const getLoanQuery = `
        query GetLoan($loanId: ID!) {
            loan(loanId: $loanId) {
                success
                message
                loan {
                    id
                    borrowerId
                    amountRequested
                    termMonths
                    interestRate
                    status
                }
            }
        }
    `;

    // GraphQL Mutation to update a loan
    const updateLoanMutation = `
        mutation UpdateLoan($loanId: ID!, $input: LoanUpdateInput!) {
            updateLoan(loanId: $loanId, input: $input) {
                success
                message
                loan {
                    id
                    status
                }
            }
        }
    `;

    const fetchLoanData = async () => {
        const token = localStorage.getItem('accessToken');
        if (!token) {
            console.error('Authentication token not found.');
            window.location.href = 'login.html';
            return;
        }

        try {
            const response = await fetch(API_URL, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                body: JSON.stringify({
                    query: getLoanQuery,
                    variables: { loanId: loanId }
                })
            });

            if (!response.ok) {
                if (response.status === 401) {
                    localStorage.removeItem('accessToken');
                    alert('Session expired or unauthorized. Please log in again.');
                    window.location.href = 'login.html';
                    return;
                }
                const errorText = await response.text();
                throw new Error(`HTTP error ${response.status}: ${errorText}`);
            }

            const result = await response.json();

            if (result.errors) {
                console.error('GraphQL Errors:', result.errors);
                formMessage.textContent = `Error fetching loan data: ${result.errors[0].message}`;
                formMessage.className = 'mt-4 text-sm font-bold text-red-500';
                return;
            }

            const loan = result.data?.loan?.loan;
            if (loan) {
                populateForm(loan);
            } else {
                formMessage.textContent = 'Loan not found.';
                formMessage.className = 'mt-4 text-sm font-bold text-red-500';
            }

        } catch (error) {
            console.error('Error fetching loan:', error);
            formMessage.textContent = 'Error connecting to the server.';
            formMessage.className = 'mt-4 text-sm font-bold text-red-500';
        }
    };

    const populateForm = (loan) => {
        borrowerIdInput.value = loan.borrowerId || '';
        amountRequestedInput.value = loan.amountRequested || '';
        termMonthsInput.value = loan.termMonths || '';
        interestRateInput.value = loan.interestRate || '';
        statusSelect.value = loan.status || 'pending';
    };

    updateLoanForm.addEventListener('submit', async (event) => {
        event.preventDefault();

        const token = localStorage.getItem('accessToken');
        if (!token) {
            formMessage.textContent = 'Authentication token not found. Please log in again.';
            formMessage.className = 'mt-4 text-sm font-bold text-red-500';
            window.location.href = 'login.html';
            return;
        }

        formMessage.textContent = 'Updating loan...';
        formMessage.className = 'mt-4 text-sm font-bold text-blue-500';

        const loanUpdateData = {
            amountRequested: parseFloat(amountRequestedInput.value),
            termMonths: parseInt(termMonthsInput.value, 10),
            interestRate: parseFloat(interestRateInput.value),
            status: statusSelect.value
        };

        // Validate inputs (optional, can be done on backend too)
        if (isNaN(loanUpdateData.amountRequested) || isNaN(loanUpdateData.termMonths) || isNaN(loanUpdateData.interestRate)) {
            formMessage.textContent = 'Please enter valid numbers for amount, term, and interest rate.';
            formMessage.className = 'mt-4 text-sm font-bold text-red-500';
            return;
        }

        try {
            const response = await fetch(API_URL, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                body: JSON.stringify({
                    query: updateLoanMutation,
                    variables: {
                        loanId: loanId,
                        input: loanUpdateData
                    }
                })
            });

            if (!response.ok) {
                if (response.status === 401) {
                    localStorage.removeItem('accessToken');
                    alert('Session expired or unauthorized. Please log in again.');
                    window.location.href = 'login.html';
                    return;
                }
                const errorText = await response.text();
                throw new Error(`HTTP error ${response.status}: ${errorText}`);
            }

            const result = await response.json();

            if (result.errors) {
                console.error('GraphQL Errors:', result.errors);
                const errorMessage = result.errors[0]?.message || 'Unknown GraphQL error';
                if (result.errors.some(e => e.message.includes('Not authorized') || e.extensions?.code === 'UNAUTHENTICATED')) {
                    alert('You do not have permission to update loan products.');
                    // window.location.href = 'dashboard.html'; // Redirect if unauthorized
                } else {
                    formMessage.textContent = `Error: ${errorMessage}`;
                    formMessage.className = 'mt-4 text-sm font-bold text-red-500';
                }
                return;
            }

            const updateResult = result.data?.updateLoan;

            if (updateResult?.success) {
                formMessage.textContent = updateResult.message || 'Loan updated successfully!';
                formMessage.className = 'mt-4 text-sm font-bold text-green-500';
                setTimeout(() => {
                    window.location.href = 'loan_product.html';
                }, 1500);
            } else {
                formMessage.textContent = updateResult?.message || 'Failed to update loan.';
                formMessage.className = 'mt-4 text-sm font-bold text-red-500';
            }

        } catch (error) {
            console.error('Error updating loan:', error);
            formMessage.textContent = error.message || 'An unexpected error occurred.';
            formMessage.className = 'mt-4 text-sm font-bold text-red-500';
        }
    });

    // Initial fetch to populate the form
    fetchLoanData();

    // Basic logout functionality (copied from other pages)
    document.getElementById('logout-btn').addEventListener('click', () => {
        alert('Logged out!'); // Replace with actual logout logic
        window.location.href = 'login.html'; // Redirect to login page
    });

    // Sidebar dropdowns
    const customerDropdownBtn = document.getElementById('customer-dropdown-btn');
    const customerDropdownMenu = document.getElementById('customer-dropdown-menu');
    if (customerDropdownBtn && customerDropdownMenu) {
        customerDropdownBtn.addEventListener('click', () => {
            customerDropdownMenu.classList.toggle('hidden');
        });
    }

    const savingsDropdownBtn = document.getElementById('savings-dropdown-btn');
    const savingsDropdownMenu = document.getElementById('savings-dropdown-menu');
    if (savingsDropdownBtn && savingsDropdownMenu) {
        savingsDropdownBtn.addEventListener('click', () => {
            savingsDropdownMenu.classList.toggle('hidden');
        });
    }
    
    const loanDropdownBtn = document.getElementById('loan-dropdown-btn');
    const loanDropdownMenu = document.getElementById('loan-dropdown-menu');
    if (loanDropdownBtn && loanDropdownMenu) {
        loanDropdownBtn.addEventListener('click', () => {
            loanDropdownMenu.classList.toggle('hidden');
        });
    }
});
